...
@implementation NSData (LGZIP)

- (NSData *)log_gzippedDataWithCompressionLevel:(float)level
{
    if ([self length])
    {
        z_stream stream;
...
- (NSData *)log_gzippedData
{
    return [self log_gzippedDataWithCompressionLevel:-1.0f];
}

- (NSData *)log_gunzippedData
{
    if ([self length])
    {
        z_stream stream;
